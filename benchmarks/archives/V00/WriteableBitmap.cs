...
public sealed class WriteableBitmap : IDisposable
{
    ...
    public bool TryBeginWrite(out WriteLease lease)
    {
        lock (_lifetimeGate)
        {
            ...
            for (var offset = 0; offset < _buffers.Length; offset++)
            {
                ...
                if (index == _frontBufferIndex || slot.InFlightImages != 0 || slot.DisposeQueued)
                    continue;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public Span<byte> ToSpanByte() => GetRawBitmap().GetPixelSpan();
        ...
        bool clearBeforeDraw)
    {
        ...
        lock (_lifetimeGate)
        {
            ...
            var slot = _buffers[_frontBufferIndex];
            var pixmap = slot.Pixmap;
            if (pixmap is null)
                return BitmapDrawResult.Unavailable;

            // DrawBitmap snapshots mutable bitmaps and copies the complete frame.
            // A fresh raster image shares this immutable front buffer without that copy.
            image = SKImage.FromPixels(pixmap, slot.ImageRelease);
            if (image is null)
                return BitmapDrawResult.Unavailable;
    ...
    {
        var bitmap = GetRawBitmap();
        var pixels = bitmap.GetPixels(out var nativeLength);
        length = checked((int)nativeLength.ToInt64());
        return pixels;
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Span<byte> GetWriteSpan(int bufferIndex, long generation)
    {
        lock (_lifetimeGate)
        {
            if (_isDisposed != 0 ||
                _activeWriteBufferIndex != bufferIndex ||
                _activeWriteGeneration != generation)
            {
                throw new ObjectDisposedException(nameof(WriteLease));
            }

            return _buffers[bufferIndex].Bitmap!.GetPixelSpan();
        }
    }
    ...
    private sealed class BufferSlot
    {
        ...
        public BufferSlot(WriteableBitmap owner, int index, SKImageInfo info)
        {
            _owner = owner;
            Index = index;
            ImageRelease = HandleImageRelease;
        ...
        private void HandleImageRelease(IntPtr _, object __) =>
