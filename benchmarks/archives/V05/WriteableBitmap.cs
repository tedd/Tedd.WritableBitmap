...
public sealed class WriteableBitmap : IDisposable
{
        ...
        bool clearBeforeDraw)
    {
        ...
        lock (_lifetimeGate)
        {
            ...
            {
                // The image shares immutable front-buffer storage. Retain its identity across
                // redraws so Skia can also reuse a GPU upload for an unchanged frame.
            ...
            // Protect the managed wrapper while DrawImage runs outside the producer lock.
            slot.ActiveDraws++;
        }
